...
from model import R2NCNN

def parse_args():
    parser = argparse.ArgumentParser(description='R2N-CNN')
...
def get_logger():
    return TensorBoardLogger('../logs', name='R2N-CNN')
...
def main():
    ...
    for train_index, val_index in folds:
        ...
        model = LightningModel(
            R2NCNN(dims, args.n_hidden, args.n_main),
    ...
    # Predict
    train_dataloader = loader_maker.make(None)
    ...
    test_dataloader = loader_maker.make(None, train=False)
    ...
    df_submission.to_csv('../submission/15_r2n-cnn.csv', index=False)
